...
//
  ...
  [_L2] =
    KEYMAP(
      ...
      _______,  DRZ__CA,  _______,  _______,  _______,  DRZ__ST,  _______,  _______,  _______,  KC_DELT,  KC_HOME,  KC_PGDN,  KC_END,   DRZ__CA,  _______
    ),
...
const macro_t *action_get_macro(keyrecord_t *record, uint8_t id, uint8_t opt) {
    ...
    case DRZ_MACRO_EVE:
      if(IS_PRESSED(record->event)) {
        return MACRO(D(LCTL), T(UP), U(LCTL), T(ENTER), END);
      }
    break;
...
bool process_overides(uint16_t keycode, keyrecord_t *record) {
  ...
  switch(keycode) {
    ...
    case FR_0:  //(
      ...
      break;
    case DRZ_CLN:
      returnValue = override_key(record, DRZ_CLN, FR_SCLN);
      break;
    case DRZ_DOT:
      returnValue = override_key(record, DRZ_DOT, FR_COMM);
      break;
    ...
    case DRZ_SLA:
      returnValue = override_key(record, DRZ_SLA, KC_BSLASH);
      break;
    case DRZ_CLN:
      returnValue = override_key(record, DRZ_CLN, KC_SCOLON);
      break;
    case DRZ_DOT:
